...
export default function CondoCard(props) {
    ...
    return ( <
        ...
        <
        Link href = { `/off-plan-properties/${props.city.slug}/${props.slug}` }
        className = "mylinkk"
        target = "_blank" >
        {
        ...
        /Link> { /* <span className="p-1 px-2 abs1">Preconstruction</span> */ } {
        ...
        span className = "px-2 abs2" > { props.no + 1 ? props.no + 1 + " " : " " } <
        /span> <
        /div> <
        Link href = { `/off-plan-properties/${props.city.slug}/${props.slug}` }
        ...
        h4 className = "mb-2 cardd-subtitle" > { checkPricing(props.price_starting_from) } <
        /h4> <
        p className = "mb-0 project-address-card" > { props.project_address } < /p> <
        p className = "card-secondary-title mb-0" > { props.project_type } | Completion { props.occupancy } <
        /p> <
        p className = "card-secondary-title2 mb-0" > { `Updated on: ${
