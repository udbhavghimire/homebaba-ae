...
export async function generateMetadata({ params }, parent) {
    ...
    return {
        ...parent,
        alternates: {
            canonical: `https://homebaba.ae/off-plan-properties/${params.city}/townhomes/`,
        },
        title: data.preconstructions.length + " Preconstruction Townhomes in " + city,
        openGraph: {
            images: retImage(data.preconstructions),
        },
        description: `${city} upcoming Off Plan Townhomes. Check out ${data.preconstructions.length}+ new construction townhomes on Dolphy. Floor plans & pricing updated for upcoming new construction townhomes in ${city}`,
    };
}
...
export default async function Home({ params }) {
    ...
    return ( <
        ...
        p className = "text-dark text-center text-md-start mb-2" > { `${
                data.preconstructions.length
              } Off Plan Townhomes in ${CapitalizeFirst(
                params.city
              )} (Updated ${
                new Date().getMonth() +
                1 +
        ...
        div >
        <
        Link className = "link-black badge py-2 bg-white shadow-sm text-dark fs-small fw-m"
        ...
        <
        Link className = "link-black badge py-2 bg-white shadow-sm text-dark fs-small fw-m"
        href = { `/off-plan-properties/${params.city}/detached/` } >
        { CapitalizeFirst(params.city) }
        Detached Homes <
        /Link> <
        Link className = "link-black badge py-2 bg-white shadow-sm text-dark fs-small fw-m"
        href = { `/off-plan-properties/${params.city}/condos/` } >
        { CapitalizeFirst(params.city) }
        Condos <
        /Link> <
        /div> <
        ...
        h3 className = "fs-2" >
        <
        strong > The Dolphy Insights < /strong> - Know Whats Happening in{" "} { CapitalizeFirst(data.city.name) } <
        /h3> <
        p >
        Learn about the new projects, news and insights and current new trends happening in { CapitalizeFirst(data.city.name) } <
        /p> <
        /div> <
        ...
        <
        BottomContactForm proj_name = "City Page"
        city = { data.city.name } >
        < /BottomContactForm> <
        /div> <
        div className = "col-md-3" > < /div> <
        ...
        div className = "py-5 max-w-mine" > {
            data.city && ( <
                ...
                <
                div className = "col-12 mt-mine px-3"
                dangerouslySetInnerHTML = {
                    {
