...
export async function generateMetadata({ params }, parent) {
    ...
    return {
        ...parent,
        alternates: {
            canonical: `https://homebaba.ae/off-plan-properties/${params.city}/condos/`,
        },
        title: data.preconstructions.length + " Preconstruction Condos in " + city,
        openGraph: {
            images: retImage(data.preconstructions),
        },
        description: `${city} upcoming Off Plan Condos. Check out ${data.preconstructions.length}+ new construction condos on Dolphy. Floor plans & pricing updated for upcoming new construction condos in ${city}`,
    };
}
...
export default async function Home({ params }) {
    ...
    return ( <
        ...
        p className = "text-dark text-center text-md-start mb-2" > { `${
                data.preconstructions.length
              } Off Plan Condos in ${CapitalizeFirst(
                params.city
              )} (Updated ${
                new Date().getMonth() +
                1 +
