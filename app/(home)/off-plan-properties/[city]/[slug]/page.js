...
async function getData(slug) {
    const res = await fetch(
        "https://api.homebaba.ae/api/preconstructions-detail/" + slug, {
            next: { revalidate: 10 },
        }
    );
...
async function getRelatedData(city) {
    const res = await fetch("https://api.homebaba.ae/api/related-precons/" + city, {
        next: { revalidate: 10 },
    });
...
const retImage = (data) => {
    if (data.image.length > 0) {
        return `https://api.homebaba.ae${data.image[0].image}`;
    } else {
...
export async function generateMetadata({ params }, parent) {
    ...
    return {
        ...parent,
        alternates: {
            canonical: `https://homebaba.ae/off-plan-properties/${params.city}/${params.slug}`,
        },
        openGraph: {
...
export default async function Home({ params }) {
    ...
    const accordionData = [{
            ...
            content: ( <
                strong > { data.project_name }
                is developed by { data.developer.name } <
                /strong>
            ),
        },
        {
            ...
            content: ( <
                strong > { data.project_name }
                is located in { data.project_address } <
                /strong>
            ),
        },
        {
            title: "What is the starting price for the homes or unit in " +
                data.project_name +
                " ?",
            content: ( <
                strong >
                The price of the homes or unit could change.Please contact the real estate agent { " " } <
                /strong>
            ),
        },
    ];
    ...
    return ( <
        ...
        <
        h1 className = "main-title text-red fw-mine" > { data.project_name } <
        /h1> <
        p className = "mb-0" >
        Developed By { " " } <
        strong >
        ...
        href = { `/off-plan-properties/builders/${data.developer.slug}/` } >
        { data.developer.name } <
        /Link> <
        /strong> <
        /p> <
        h2 className = "vmain-title fs-3 fw-mine3 mt-1 mb-0" > { checkPricing(data.price_starting_from, data.price_to) } <
        /h2> <
        div className = "mb-1" >
        <
        ...
        <
        /span> <
        span scope = "col" > { data.project_address }, { data.city.name } <
        /span> <
        /div> <
        /div> <
        /div> <
        ...
        <
        img alt = "Register Now Text Design"
        src = "/reg.webp"
        ...
        <
        Link href = "telto:(587) 887-2572"
        className = "link-black" >
        <
        i className = "bi bi-telephone" > < /i> (587) 887-2572 <
        /Link> <
        /p> <
        p className = "mb-0 text-center" > hello @homebaba.ae < /p> <
        /div> <
        /div> <
        div className = "my-4" > < /div> <
        SideContactForm proj_name = { data.project_name }
        city = { data.city.name }
        defaultmessage = {
        ...
        <
        BottomContactForm proj_name = { data.project_name }
