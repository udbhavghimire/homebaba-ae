...
async function getData(developer) {
    const res = await fetch(
        "https://api.homebaba.ae/api/preconstructions-developer/" + developer, {
            next: { revalidate: 10 },
        }
    );
    if (!res.ok) {
...
const retImage = (data) => {
    if (data.image) {
        return `https://api.homebaba.ae${data.image}`;
    } else {
...
export async function generateMetadata({ params }, parent) {
    ...
    return {
        ...parent,
        alternates: {
            canonical: `https://homebaba.ae/off-plan-properties/builders/${params.builder}/`,
        },
        openGraph: {
            images: retImage(all_data.developer),
        },
        title: city + "- New home developer and builder | Communities & Developments",
        description: "Search our selection of off plan properties for sale by " +
            city +
            ". " +
            city +
            "'s ever-changing portfolio of Off Plans brings you closer to your ideal homes",
    };
}
...
export default async function BuilderSingle({ params }) {
    ...
    return ( <
        ...
        Check out currently Selling, Upcoming or Past Communities by 3 bridges properties <
        br / >
        Check out plans, pricing, availability
        for preconstruction homes by { CapitalizeFirst(params.builder) } <
        /p> < /
        div > <
        div className = "col-md-2" > < /div> < /
        div > <
        ...
        <
        BottomContactForm proj_name = { params.builder }
