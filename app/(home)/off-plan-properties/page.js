...
export async function generateMetadata({ params }, parent) {
    return {
        ...parent,
        alternates: {
            canonical: `https://homebaba.ae/off-plan-properties/`,
        },
        openGraph: {
            images: "/social/precon.webp",
        },
        title: `Be First to Move Into UAE's Most Exciting New Construction Neighborhoods`,
        description: `Lock in pricing and incentives on model homes available for a limited time. Act now before inventory fills up in UAE's most in-demand locales. With prices still accessible compared to resales, find your perfect fit.`,
    };
}
...
export default async function Home({ params }) {
    ...
    return ( <
        ...
        List of Off Plan Projects in < br / > UAE <
        /h1> <
        ...
        div className = "row row-cols-1 row-cols-md-1 gy-4" > {
            ...
            all_data.map((item) => ( <
                div className = "col" >
                <
                Link href = { "/off-plan-properties/" + item.slug }
                ...
                div className = "row row-cols-2 row-cols-md-5" > {
                    ...
                    item.preconstructions.map((precon, no) => ( <
                        Link href = { `/off-plan-properties/${item.slug}/${precon.slug}` }
                        className = "mb-0 fs-small col"
                        target = "_blank" > { precon.project_name } <
                        /Link>
                    ))
                } <
                /div> < /
        ...
        div > {
            ...
            cities.map((item) => ( <
                Link href = { "/off-plan-properties/" + item.slug }
                className = "btn btn-light link-black me-3 mb-3" > { item.name } <
                /Link>
            ))
        } <
        /div> < /
